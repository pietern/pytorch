#include "python_compiler_mixin.h"
...
struct CompiledFunction {
  ...
  PyObject* call(PyObject *pyargs) {
    if (!enabled_) {
      return PyObject_CallObject(function_.get(), pyargs);
    }
    auto args = flatten(pyargs); // TODO: add captured vars?
    auto& ktrace = getTrace(args);

    variable_list out_vars;
    if (ktrace.ready()) {
      hits_++;
      return unflatten(ktrace.run(args.vars), ktrace.out_desc_);
    } else {
      misses_++;
      return ktrace.add_trace(pyargs, args.vars);
    }
  }

  bool hasTraceFor(PyObject *pyargs) {
    auto args = flatten(pyargs); // TODO: add captured vars?
    auto it = ktraces_.find(args.desc);
    if (it == ktraces_.end())
      return false;
  ...
  CompiledFunction(int nderivs, bool optimize, py::object function,
                   variable_list captured_vars, std::string name)
    : nderivs_(nderivs)
    , optimize_(optimize)
    , enabled_(true)
    , hits_(0)
    ...
    , name_(std::move(name))
    , captured_vars_(std::move(captured_vars))
    , ktraces_() {}
...
void initCompilerMixin(PyObject *module) {
  ...
  py::class_<CompiledFunction>(m, "CompiledFunction")
    .def(py::init<int, bool, py::object, variable_list, std::string>())
    .def("__call__", [](CompiledFunction& fn, py::args& args) -> py::object {
      return py::reinterpret_steal<py::object>(fn.call(args.ptr()));
    })
    .def("has_trace_for", [](CompiledFunction& fn, py::args& args) -> bool {
      return fn.hasTraceFor(args.ptr());
    })
