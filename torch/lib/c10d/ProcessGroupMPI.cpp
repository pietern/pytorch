...
std::shared_ptr<ProcessGroupMPI>
ProcessGroupMPI::createProcessGroupMPI(std::vector<int> ranks) {
  // Once initialization
  initMPIOnce();
...
ProcessGroupMPI::ProcessGroupMPI(int rank, int size, MPI_Comm pgComm)
    : ProcessGroup(rank, size), stop_(false), pgComm_(pgComm), groupRank_(-1),
      groupSize_(-1) {
  std::unique_lock<std::mutex> globalLock(pgGlobalMutex_);
  ...
  if (pgComm_ != MPI_COMM_NULL) {
    ...
    MPI_CHECK(MPI_Allgather(rankToGroupRank.data(), 2, MPI_INT,
                            allRankToGroupRank.data(), 2, MPI_INT, pgComm_));
...
ProcessGroupMPI::~ProcessGroupMPI() { destroy(); }
...
std::shared_ptr<ProcessGroup::Work>
ProcessGroupMPI::enqueue(std::unique_ptr<WorkEntry> entry) {
...
std::shared_ptr<ProcessGroup::Work>
ProcessGroupMPI::broadcast(std::vector<at::Tensor> &tensors,
...
std::shared_ptr<ProcessGroup::Work>
ProcessGroupMPI::allreduce(std::vector<at::Tensor> &tensors,
  ...
  std::function<void(std::unique_ptr<WorkEntry> &)> runFunc =
      [opts, this](std::unique_ptr<WorkEntry> &entry) {
        auto data = (entry->src)[0];
        std::unique_lock<std::mutex> globalLock(pgGlobalMutex_);
        MPI_CHECK(MPI_Allreduce(MPI_IN_PLACE, data.data_ptr(), data.numel(),
                                mpiDatatype.at(data.type().scalarType()),
...
std::shared_ptr<ProcessGroup::Work>
ProcessGroupMPI::reduce(std::vector<at::Tensor> &tensors,
...
std::shared_ptr<ProcessGroup::Work>
ProcessGroupMPI::allgather(std::vector<std::vector<at::Tensor>> &outputTensors,
  ...
  std::function<void(std::unique_ptr<WorkEntry> &)> runFunc = [this](
    ...
    MPI_CHECK(MPI_Allgather(data.data_ptr(), data.numel(),
                            ...
                            mpiDatatype.at(data.type().scalarType()), pgComm_));
...
std::shared_ptr<ProcessGroup::Work>
ProcessGroupMPI::gather(std::vector<std::vector<at::Tensor>> &outputTensors,
                        std::vector<at::Tensor> &inputTensors,
  ...
  std::function<void(std::unique_ptr<WorkEntry> &)> runFunc = [opts, this](
      std::unique_ptr<WorkEntry> &entry) {
    auto data = (entry->src)[0];
    ...
    MPI_CHECK(MPI_Gather(data.data_ptr(), data.numel(),
                         mpiDatatype.at(data.type().scalarType()), recvbuf,
                         data.numel(), mpiDatatype.at(data.type().scalarType()),
                         opts.rootRank, pgComm_));
...
std::shared_ptr<ProcessGroup::Work>
ProcessGroupMPI::scatter(std::vector<at::Tensor> &outputTensors,
  ...
  std::function<void(std::unique_ptr<WorkEntry> &)> runFunc =
      [opts, this](std::unique_ptr<WorkEntry> &entry) {
        ...
        MPI_CHECK(MPI_Scatter(
            sendbuf, data.numel(), mpiDatatype.at(data.type().scalarType()),
            data.data_ptr(), data.numel(),
            mpiDatatype.at(data.type().scalarType()), opts.rootRank, pgComm_));
      };
...
std::shared_ptr<ProcessGroup::Work>
ProcessGroupMPI::send(std::vector<at::Tensor> &tensors, int dstRank, int tag) {
  if (pgComm_ == MPI_COMM_NULL) {
    return nullptr;
  }
...

  {
    ...
    MPI_CHECK(MPI_Isend(tensor.data_ptr(), tensor.numel(),
                        mpiDatatype.at(tensor.type().scalarType()), dstRank,
                        tag, pgComm_, &request));
  }
...
std::shared_ptr<ProcessGroup::Work>
ProcessGroupMPI::recv(std::vector<at::Tensor> &tensors, int srcRank, int tag) {
  if (pgComm_ == MPI_COMM_NULL) {
    return nullptr;
  }
...

  {
    ...
    MPI_CHECK(MPI_Irecv(tensor.data_ptr(), tensor.numel(),
                        mpiDatatype.at(tensor.type().scalarType()), srcRank,
                        tag, pgComm_, &request));
  }
...
std::shared_ptr<ProcessGroup::Work>
ProcessGroupMPI::recvAnysource(std::vector<at::Tensor> &tensors, int tag) {
  if (pgComm_ == MPI_COMM_NULL) {
    return nullptr;
  }
...

  {
    ...
    MPI_CHECK(MPI_Irecv(tensor.data_ptr(), tensor.numel(),
                        ...
                        MPI_ANY_SOURCE, tag, pgComm_, &request));
  }
...
std::shared_ptr<ProcessGroup::Work>
ProcessGroupMPI::barrier(const BarrierOptions &opts) {
  if (pgComm_ == MPI_COMM_NULL) {
    return nullptr;
  }
  std::function<void(std::unique_ptr<WorkEntry> &)> runFunc =
