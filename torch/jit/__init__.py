        ...
        if inspect.isclass(arg):
            class CompiledModuleMeta(type):
                def __call__(cls, *args, **kwargs):
                    ...
                    meth = obj.forward
                    compiled_forward = torch._C.CompiledFunction(nderivs, optimize,
                                                                 obj.forward,
                                                                 list(obj.parameters()),
                                                                 arg.__name__)
                    compiled_forward.enabled = enabled
                    obj.forward = compiled_forward
                    obj.has_trace_for = compiled_forward.has_trace_for
                    return obj
...
def verify(model, args, loss_fn=torch.sum, devices=None):
    ...
    # for this case
    if not isinstance(model.forward, torch._C.CompiledFunction):
